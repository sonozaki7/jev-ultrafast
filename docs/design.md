...
The model sees visible text. Background focus emulation keeps animation frames running in the owned tab. One persistent background tab per site is reused by later runs, including runs that were interrupted, so repeated checks never open duplicate tabs and the same tab stays where the owner put it (for example inside one of their own Chrome tab groups, which CDP cannot otherwise address); tabs unused for a month are closed. Screenshots are optional and disabled in library calls by default; `screenshots=True` or `record_dir=...` enables them. The inspector enables them explicitly. A continuous screencast can record a run separately.
